...
* Handlers registered without the source generator can call `services.AddNativeAotCommandInterceptors<TCommand, TResponse>()`, `AddNativeAotExclusiveCommandInterceptors`, `AddNativeAotQueryInterceptors` or `AddNativeAotStreamQueryInterceptors` after `AddPulse`.
