...
public sealed class PulseHandlerGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var combined = collected.Combine(rootNamespace);

        context.RegisterSourceOutput(combined, static (spc, data) => Execute(spc, data.Left, data.Right));
    }
    ...
    )
    {
        ...
        var source = GenerateSource(allRegistrations, rootNamespace);
        spc.AddSource("PulseHandlerRegistrations.g.cs", source);
    }

    private static string GenerateSource(List<HandlerRegistration> registrations, string? rootNamespace)
    {
        ...
        _ = sb.AppendLine(
            "        public static global::Microsoft.Extensions.DependencyInjection.IServiceCollection AddGeneratedPulseHandlers(this global::Microsoft.Extensions.DependencyInjection.IServiceCollection services)"
        );
        _ = sb.AppendLine("        {");
        ...
        foreach (var reg in registrations)
        {
            ...
            _ = sb.AppendLine(
                $"            global::Microsoft.Extensions.DependencyInjection.Extensions.ServiceCollectionDescriptorExtensions.{methodName}<{reg.ServiceTypeName}, {reg.HandlerTypeName}>(services);"
            );
        }
