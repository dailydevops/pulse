...
/// <summary>
/// Provides a shared SQL Server container for SQL Server integration tests.
...
#pragma warning disable CA2100 // Review SQL queries for security vulnerabilities - Script comes from trusted embedded resource
    ...
    {
        var scriptPath = Path.Combine(AppContext.BaseDirectory, "Scripts", "SqlServer", "OutboxMessage.sql");
...
#pragma warning restore CA2100
    ...
    /// <summary>
    /// Initializes a minimal outbox schema without schema prefix, for tests that rely on plain table names.
    /// </summary>
    /// <param name="databaseName">The database name.</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    [SuppressMessage(
        "Security",
        "CA2100:Review SQL queries for security vulnerabilities",
        Justification = "DDL is a constant string with no user input."
    )]
    public async Task InitializeOutboxSchemaAsync(string databaseName, CancellationToken cancellationToken = default)
    {
        await using var connection = new SqlConnection(GetConnectionString(databaseName));
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
